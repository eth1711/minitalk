...
/*   Updated: 2023/07/24 13:24:42 by ethanlim         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int    ft_printf(const char *format, ...)
{
	...
	while (format[++i] != '\0')
	{
		if (format[i] == '%')
		{
			...
			{
				ft_printnum((long long int)va_arg(args, unsigned long int), "0123456789abcdef", &total);
			}
			else if (format[i + 1] == 'p')
			{
				total += write(1, "0x", 2);
				ft_printnum_unsigned((long int)va_arg(args, unsigned long int), "0123456789abcdef", &total);
			}
			else if (format[i + 1] == 'X')
			{	
				ft_printnum((long int)va_arg(args, unsigned long int), "0123456789ABCDEF", &total);
			}
			else if (format[i + 1] == '%')
				ft_printchar('%', &total);
			i++;
